...
# ===== 추가: 원문에 없는 월/일/가격/고유명사(메뉴 등) 제거용 =====
RE_MD_FULL   = re.compile(r'(?:1[0-2]|0?[1-9])\s*월\s*(?:[12]?[0-9]|3[01])\s*일')
RE_MD_MONTH  = re.compile(r'(?:1[0-2]|0?[1-9])\s*월')
RE_MD_DAY    = re.compile(r'(?:[12]?[0-9]|3[01])\s*일')
RE_MD_RANGE  = re.compile(r'(?:[12]?[0-9]|3[01])\s*~\s*(?:[12]?[0-9]|3[01])\s*일')
RE_PRICE     = re.compile(r'(?<!\d)(\d{1,3}(?:,\d{3})*(?:\.\d+)?)\s*(만원|원|만원대|원대)\b')
RE_FOOD_NEAR = re.compile(r'(?:특히\s+)?(?P<word>[가-힣]{2,10})(?:[이가은는을를])?\s*(?=(인상|맛있|훌륭|좋았|판매|제공|유명|추천))')

def _normalize_ws(s: str) -> str:
    return re.sub(r'\s+', '', s or '')

def _extract_allowed_md_price(src: str):
    allowed = set()
    for pat in (RE_MD_FULL, RE_MD_RANGE, RE_MD_MONTH, RE_MD_DAY, RE_PRICE):
        for m in pat.finditer(src or ""):
            allowed.add(_normalize_ws(m.group(0)))
    return allowed

def _sanitize_against_src(summary: str, src: str) -> str:
    """원문에 없는 월/일/기간/가격, 그리고 메뉴/고유명사(제한적) 제거. 각 샘플당 한 번만 호출."""
    if not summary:
        return summary or ""
    src_text = src or ""
    allowed_md_price = _extract_allowed_md_price(src_text)

    def _filter_pat(text: str, pat: re.Pattern):
        return pat.sub(lambda m: m.group(0) if _normalize_ws(m.group(0)) in allowed_md_price else "", text)

    out = summary
    for pat in (RE_MD_FULL, RE_MD_RANGE, RE_MD_MONTH, RE_MD_DAY, RE_PRICE):
        out = _filter_pat(out, pat)

    allowed_words = set(re.findall(r"[가-힣]{2,}", src_text))
    def _drop_fake_menu(m: re.Match):
        w = m.group("word")
        return m.group(0) if w in allowed_words else ""
    out = RE_FOOD_NEAR.sub(_drop_fake_menu, out)
...
def _llm_summarize(title: str, addr: str, review: str) -> str:
    ...
    messages = [
        {"role": "system", "content":
         ...
         "- 입력에 없는 날짜(월/일/기간), 가격/수치, 고유명사 생성 금지.\n"  # ← 추가
         "- 출력은 한국어 문장으로만 작성."
        },
        {"role": "user",
    ...
    try:
        ...
        out1 = _sanitize_against_src(out1, review)  # ← 추가: 한 번만 호출
    ...
    try:
        ...
        out2 = _sanitize_against_src(out2, review)  # ← 추가: 한 번만 호출
...
def periodic_refresh(db, interval_days=180, batch_size=10, write=True):
    import datetime as _dt
    import time as _time
    while True:
        try:
            now = _dt.datetime.utcnow()
            cutoff_dt = now - _dt.timedelta(days=interval_days)
            places_iter = db.collection("places").stream()
            for place_doc in places_iter:
                pid = place_doc.id
                sum_ref = db.collection("places").document(pid).collection("meta").document("summary")
                sum_doc = sum_ref.get()
                need = True
                if sum_doc.exists:
                    ts = (sum_doc.to_dict() or {}).get("updatedAt")
                    if ts is None:
                        need = True
                    elif hasattr(ts, "timestamp"):
                        need = ts.timestamp() <= cutoff_dt.timestamp()
                ...
                if not need:
                    continue
                rev_iter = db.collection("places").document(pid).collection("reviews").stream()
                reviews = []
                for rdoc in rev_iter:
                    text = ((rdoc.to_dict() or {}).get("review") or "").strip()
                    if text:
                        reviews.append((rdoc.reference, text))
                if not reviews:
                    continue
                joined = " ".join(t for _, t in reviews)[:4000]
                pdata = place_doc.to_dict() or {}
                title = pdata.get("placeName") or f"장소 {pid}"
                addr = pdata.get("region") or ""
                ...
                payload = {
                    ...
                    "title": title,
                    "region": addr,
                    "sourceReviewIds": [r.id for r, _ in reviews],
                }
                if write:
                    sum_ref.set(payload, merge=True)
            _time.sleep(interval_days * 24 * 3600)
...
if __name__ == "__main__":
    ...
    db_periodic = _init_db(args.emulator, args.project)
    threading.Thread(target=periodic_refresh, args=(db_periodic, 180, args.batch_size, args.write), daemon=True).start()
